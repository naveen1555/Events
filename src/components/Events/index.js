...
class Events extends Component {
  ...
  getRenderEventRegistrationStatusDetails = () => {
    const {activeEventId} = this.state

    const getActiveEventDetails = eventsList.find(
      eachEvent => eachEvent.id === activeEventId,
    )
    if (getActiveEventDetails) {
      return getActiveEventDetails.registrationStatus
    }
    return ''
  }
      ...
      <ul className="events-list-container">
        {eventsList.map(eachEvent => (
          <EventItem
            ...
            clickedEventDetails={this.clickedEventDetails}
    ...
    return (
      <div className="container">
        ...
        <ActiveEventRegistrationDetails
          renderEventRegistrationDetails={this.getRenderEventRegistrationStatusDetails()}
        />
      </div>
    )
  }
